...
evalProgStepwise program regarray label output =
  case program `instrLabeled` label of
    ...
    HALT            -> let ((newregarray, nextlabel), newoutput) = evalInstr HALT regarray label output in
                         "Execute instruction labeled " ++ show label ++ ": " ++ show HALT ++
                         "\nwith register(s): " ++ showRegArray regarray ++
  ...
  do program <- readFile "Happy.txt"
     case parse rmParser "" program of
       ...
       Right r -> case r of
                    ...
                    HALT     -> putStrLn (exec [HALT] "")
                    Seq s    -> putStrLn (exec s "")
----------------------------------------------------------------------------------------
...
ifInstr =
  do reserved "IFEMPTY"
     ...
     reserved "OR"
     label15 <- natural
     return $ JUMP register [label1, label2, label3, label4, label5, label6, label7, label8, label9, label10, label11, label12, label13, label14, label15]
